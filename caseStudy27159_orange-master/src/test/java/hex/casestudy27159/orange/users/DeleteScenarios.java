...
public class DeleteScenarios {
	...
	public void deleteExistentUser() {
		...
		WebElement user = UserManagementForm.UserCheckboxByUserName(driver, "Alice.Duval");
		Assert.assertTrue(user.isDisplayed());
		
		if(user.isDisplayed()) {
			FindElement.Click(driver, user);
			
			WebElement deleteButton = UserManagementForm.DeleteButton(driver);
			FindElement.Click(driver,deleteButton);
			
			WebElement okButton = UserManagementForm.ConfirmationModal.OKDeleteButton(driver);
			Assert.assertTrue(okButton.isDisplayed());
			FindElement.Click(driver, okButton);
			
			WebElement label = UserManagementForm.messagesLabel(driver);
			Assert.assertTrue(label.isDisplayed());
			FindElement.Wait(1);
			Assert.assertEquals(label.getText(), "Successfully Deleted");
		} 
